...
function updatePagination() {
  ...
  if (currentPage > 3) {
    pagination.innerHTML += `<li class="page-item"><a class="page-link" href="fav?id=${id}&page=1">1</a></li>`;
    if (currentPage > 4) {
  ...
  for (let i = Math.max(1, currentPage - 2); i <= Math.min(totalPages, currentPage + 2); i++) {
    pagination.innerHTML += `<li class="page-item ${i === currentPage ? 'active' : ''}"><a class="page-link" href="fav?id=${id}&page=${i}">${i}</a></li>`;
  }
  ...
  if (currentPage < totalPages - 2) {
    ...
    pagination.innerHTML += `<li class="page-item"><a class="page-link" href="fav?id=${id}&page=${totalPages}">${totalPages}</a></li>`;
  }
  ...
  document.getElementById('prev-page').addEventListener('click', (e) => {
    ...
    if (currentPage > 1) {
      currentPage--;
      window.location.href = `fav?id=${id}&page=${currentPage}`;
    }
  });
